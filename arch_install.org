...
Check the default sub-volume,

#+begin_src bash
  $ btrfs subvol get-default /
#+end_src

If not our "@" sub-volume, list all to find its ID and then set it as the
default. A =grub-install= must follow this.. Example:

#+begin_src bash
  $ btrfs subvol set-default 258 /
#+end_src
