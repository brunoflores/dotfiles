...
#+begin_src emacs-lisp
  (use-package general
    :after evil
    :config
    ;; SPC in normal state always opens hub transient
    ;; This works even in dired-mode
    (general-define-key
     :states 'normal
     :keymaps 'override
     "SPC" #'my/hub-transient-dispatch))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/hub-transient-layout ()
    "Return the list of transient groups (columns) for the hub.
  This is computed dynamically based on `major-mode`."
    (let* (;; Always-present global columns
  	 (global-groups
  	  '([["Files"
  	      ("f" "Files"   my/files-transient)
  	      ("b" "Buffers" my/buffers-transient)
  	      ("B" "Bookmarks" my/bookmarks-transient)
  	      ]
               ["Projects"
                ("p" "Project" my/project-transient)]
               ["Denote"
                ("n" "Denote" my/denote-transient)]
               ["Citar"
                ("c" "Citar" my/citar-transient)]
  	     ["Git"
  	   ...
  	   ((derived-mode-p 'org-mode)
  	    '([["Org"
  		("o" "Org menu" my/org-transient)
  		("r" "Roam menu" my/roam-transient)
  		("a" "Agenda"   org-agenda)]]))

             ;; OCaml / dune (tweak modes as needed)
  	   ((or (derived-mode-p 'tuareg-mode)
  		(derived-mode-p 'ocaml-mode))
  	    '([["OCaml / Dune"
  		("d" "Dune menu" my/ocaml-dune-transient)]]))

  	   ;; Default: no extra columns
  	   (t nil))))

      ;; Concatenate global and mode-specific groups
      (append global-groups mode-groups)))

  (defun my/hub-transient-define ()
    "Define (or redefine) `my/hub-transient` using the current layout."
    (eval
     `(transient-define-prefix my/hub-transient ()
        "Main hub menu."
        ,@(my/hub-transient-layout))))

  (defun my/hub-transient-dispatch ()
    "Regenerate and show the hub transient for the current major mode."
    (interactive)
    (my/hub-transient-define)
    (my/hub-transient))
#+end_src

** Citar transient
#+begin_src emacs-lisp
  (transient-define-prefix my/citar-transient ()
    "Citar-related commands."
    [["Citar"
      ("c" "create note" citar-create-note)
      ("n" "open note" citar-denote-open-note)
      ("x" "nocite" citar-denote-nocite)
      ("r" "add reference" citar-denote-add-reference)
      ("R" "remove reference" citar-denote-remove-reference)
      ("d" "dwim" citar-denote-dwim)
      ("e" "open reference entry" citar-denote-open-reference-entry)
      ]
     ])
#+end_src
...
#+begin_src emacs-lisp
  (transient-define-prefix my/denote-transient ()
    "Denote-related commands."
    [["Denote"
      ("n" "denote" denote)

      ("r" "rename-file" denote-rename-file
       :if-not-mode dired-mode)
      ...
      ("d" "dired" denote-dired)
      ("g" "grep" denote-grep)
      ]
     ["Dired"
      ("r" "rename-files" denote-dired-rename-files
       :if-mode dired-mode)
      ("R" "rename-marked-files-using-front-matter" denote-dired-rename-marked-files-using-front-matter
       :if-mode dired-mode)
      ("i" "link-marked-notes" denote-dired-link-marked-notes
       :if-mode dired-mode)
      ("k" "rename-marked-files-with-keywords"
...
** Bookmarks transients
#+begin_src emacs-lisp
  (transient-define-prefix my/bookmarks-transient ()
    "Bookmarks-related commands."
    [["Find"
      ("b" "Consult" consult-bookmark)
      ]
     ])
#+end_src
...
#+begin_src emacs-lisp
  (transient-define-prefix my/roam-transient ()
    ...
    [["Find / Insert"
      ...
      ("s" "Search nodes" consult-org-roam-search)
      ("l" "Forward links" consult-org-roam-forward-links)
      ("b" "Backlinks" consult-org-roam-backlinks)
      ("B" "Backlinks recursive" consult-org-roam-backlinks-recursive)
      ]
     ["Dailies"
...
#+begin_src emacs-lisp
  ...
  (defface my/org-highlight-todo
    '((t :weight bold :foreground "yellow"))
    "Face for highlighted TODO in org buffers.")
  (defface my/org-highlight-bold
    '((t :weight bold))
    "Face for highlighted 'intution' in org buffers.")

  ;; 2. Define a setup function
  (defun my/org-extra-highlights ()
    ;; Make font-lock regex matching case-insensitive in this buffer
    (setq-local font-lock-keywords-case-fold-search t)

    (font-lock-add-keywords
     nil
     '(("^[^*].*\\bintuition\\b" 0 'my/org-highlight-bold t)
       ("\\bTODO\\b" 0 'my/org-highlight-todo t))))
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    (org-mode . git-gutter-mode)
    ;; (org-mode . my/org-extra-highlights)
    (org-mode . my/org-latex-scale)
    :bind (:map org-mode-map
                ("M-n" . org-move-subtree-down)
                ("M-p" . org-move-subtree-up))
    :custom
    (org-directory "~/devel/www-roadmap/denote")
    ...
    ;; LaTeX packages for inline rendering
    ...
    (org-roam-capture-templates
     `(
       ("c" "Concept" plain
        "%?"
        :target (file+head
                 "notes/concepts/%<%Y%m%d%H%M%S>-${slug}.org"
                 "#+title: ${title}\n#+filetags: :concept:\n#+created: %U\n")
        :unnarrowed t)
       ...
       ("b" "Book note" plain
        "%?"
        :target (file+head
                 "notes/sources/%(my/ror-citekey)-%<%Y%m%d>-${slug}.org"
...
#+begin_src emacs-lisp
  (use-package citar
    ...
    ;; Org-cite
    (org-cite-global-bibliography '("~/devel/www-roadmap/bib/refs.bib"))
...
#+begin_src emacs-lisp
  (use-package denote
    ...
    :custom
    ;; Denote adds "%l": a “backlink to where I came from” - we remove that here:
    (denote-org-capture-specifiers "%?")
    :config
    (setq denote-directory (expand-file-name "~/devel/www-roadmap/denote"))
    ...
    (setq denote-known-keywords '("concept"))
